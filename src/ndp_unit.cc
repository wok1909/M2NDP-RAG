  ...
  try {
    ...
    while (k_id < ndp_kernel->num_kernel_bodies) {
      ...
      while (count * PACKET_SIZE < kinfo->size) {
        ...
        m_uthread_sync_idx.at(ndp_req_id-1) = m_sub_core_units[0]->ExecuteKernelBody(scratchpad_map, &info, k_id, uthread_sz);
...
void NdpUnit::handle_finished_context() {
  while (check_finished_context()) {
    ...
    if (context.request_info->type == KERNEL_BODY) {
      ...
      m_uthread_generator->retrieve_kernel_body(context.request_info->launch_id, context.kernel_body_id, context.uthread_id);
    }
