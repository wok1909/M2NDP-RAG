...
DATASET="siftsmall-128-euclidean"
...
class SearchGraphKernel(NdpKernel):
    def __init__(self, dataset, n_query, topk_, num_level, ef_size):
        ...
        # self.pq_size_addr = 0x1000000000200000
        # self.visited_cnt_addr = 0x1000000000300000
        # self.check_exist_addr = 0x1000000000400000
        # self.partial_cand_addr = 0x1000000000500000
        # self.partial_sum_addr = 0x1000000000700000

        self.qdata_sapd_addr = 0x10000000000000b0
        self.pq_size_addr = 0x10000000000000c0
        self.visited_cnt_addr = 0x10000000000000d0
        self.check_exist_addr = 0x1000000000000100
        self.partial_cand_addr = 0x1000000000000200
        self.partial_sum_addr = 0x1000000000000300

        # Register context
        self.register_context_addr = 0x1000000000000400
        ...
        self.num_query, self.num_data, self.num_dims, self.max_level, self.max_m, self.max_m0, _, _, self.enter_point, graphs, queries, data = read_graph_file(self.graph_file)
        ...
        # print("Graph: ")
        # for i in range(len(self.graph) // self.max_m0):
        #   for j in range(self.max_m0):
        #     print(f"{self.graph[i * self.max_m0 + j]} ", end='')
        #   print('')
        # exit()
        ...
        self.input_addrs = [self.qdata_addr, self.data_addr, self.entries_addr, self.graph_addr, self.degree_addr, self.nns_addr, self.distances_addr,
                            ...
                            self.qdata_sapd_addr, self.pq_size_addr, self.visited_cnt_addr, self.check_exist_addr, self.partial_cand_addr, self.partial_sum_addr]
    ...
    def make_kernel(self):
        ...
        # Query loop
        template += f'li x14, 0\n'  # loop idx
        template += self.store_registers(free_regs=[3, 10], scalar_regs=[4, 5, 6, 7, 8, 9, 14])
        template += f'.LOOP0\n'
        template += self.load_registers(free_regs=[3, 10], scalar_regs=[4, 7, 8, 9, 14])
        ...
        template += f'muli x21, x21, {3 * data_size}\n'
        ...
        template += f'ld x21, {self.get_arg_offset(self.pq_size_addr)}(x1)\n'
        ...
        template += f'muli x17, x14, {32 * 4 * self.num_dims}\n'
        template += f'add x23, x23, x17\n'
        ...
        template += self.store_registers(free_regs=[3, 10], scalar_regs=[15, 16, 19, 20, 22, 24, 31], vector_regs=[1])
        ### PushNodeToSearchPq Start ###
        template += f'.LOOP20\n'
        template += self.load_registers(free_regs=[17, 19], scalar_regs=[16, 24, 25])
        # CheckAlreadyExists
        template += f'li x1, {configs.spad_addr}\n'
        ...
        template += self.store_registers(free_regs=[17, 19], scalar_regs=[3, 18])
        ...
        template += f'.LOOP2\n'
        template += self.load_registers(free_regs=[17, 19], scalar_regs=[3, 18])
        ...
        ################ SYNC2 #################
        template += f'KERNELBODY:\n'  # KERNELBODY2
        template += self.load_registers(free_regs=[17, 19], scalar_regs=[3, 31])
        template += f'lw x18, (x3)\n' # exists

        # When x31 is 1 (before while loop) -> go to CheckVisited LOOP21
        # When x31 is 0 (inside while loop) -> go to SKIP20
        ...
        template += f'.SKIP70\n'
        template += self.load_registers(free_regs=[17, 19], scalar_regs=[5, 24], vector_regs=[1])
        template += f'li x1, {configs.spad_addr}\n'
        ...
        template += self.load_registers(free_regs=[17, 19], scalar_regs=[7, 16, 24])
        ...
        template += f'addi x21, x21, 1\n'
        template += f'ld x3, {self.get_arg_offset(self.pq_size_addr)}(x1)\n'
        ...
        template += f'.SKIP3\n'
        template += self.load_registers(free_regs=[17, 19], scalar_regs=[31])
        template += f'beqz x31, .SKIP20\n'  # from while loop
        ...
        ### CheckVisited Start ###
        template += f'.LOOP21\n'
        template += self.load_registers(free_regs=[17, 18], scalar_regs=[8, 9, 19, 22, 24])
        template += f'li x23, 0\n'  # ret = false
        ...
        template += self.store_registers(free_regs=[17, 18], scalar_regs=[23, 25, 29, 30])
        ################ SYNC5 #################
        template += f'KERNELBODY:\n'  # KERNELBODY5
        template += self.load_registers(free_regs=[17, 18], scalar_regs=[20, 22, 23, 24, 25, 29, 30])
        ...
        # template += self.store_register_to_all(free_regs=[17, 18], scalar_regs=[22])  # Need to broadcast to other uthreads in spad?
        template += f'j .SKIP8\n'

        template += f'.SKIP9\n'
        template += f'li x23, 1\n'
        template += f'.SKIP8\n'
        template += self.store_registers(free_regs=[17, 18], scalar_regs=[22, 23])  # Need to broadcast to other uthreads in spad?

        template += self.load_registers(free_regs=[21, 26], scalar_regs=[23, 31]) # x31 is maybe random only stored in uthread0
        ...
        ### GetCand Start ###
        template += f'.LOOP22\n'
        template += self.load_registers(free_regs=[17, 18], scalar_regs=[16])

        template += f'li x1, {configs.spad_addr}\n'
        template += f'ld x29, {self.get_arg_offset(self.partial_cand_addr)}(x1)\n'
        template += f'ld x30, {self.get_arg_offset(self.pq_size_addr)}(x1)\n'
        template += f'lw x21, (x30)\n'  # size
        ...
        template += self.store_registers(free_regs=[17, 18], scalar_regs=[25, 29, 30])
        ################ SYNC7 #################
        template += f'KERNELBODY:\n'  # KERNELBODY7
        # Global cand
        template += self.load_registers(free_regs=[17, 19], scalar_regs=[31])
        template += f'bgt UTHREADID, x0, .SKIP13\n'
        template += self.load_registers(free_regs=[17, 19], scalar_regs=[16, 29, 30])
        ...
        ################ SYNC8 #################
        template += f'KERNELBODY:\n'  # KERNELBODY8
        template += self.load_registers(free_regs=[17, 19], scalar_regs=[16, 29])

        template += f'li x31, 0\n'
        template += f'lw x25, (x29)\n' # idx
        template += self.store_registers(free_regs=[17, 18], scalar_regs=[25, 31])
        template += f'.LOOP8\n'
        template += self.load_registers(free_regs=[17, 19], scalar_regs=[16, 25])
        ...
        template += self.store_registers(free_regs=[17, 18], scalar_regs=[26])
        ################ SYNC9 #################
        template += f'KERNELBODY:\n'  # KERNELBODY9
        ...
        template += self.store_registers(free_regs=[18, 19], scalar_regs=[17, 26, 27])
        template += f'.LOOP9\n'
        ...
        template += f'j .LOOP21\n'  # CheckVisited()  -> 4
        template += f'.SKIP21\n'
        template += self.load_registers(free_regs=[18, 19], scalar_regs=[23, 26, 27])
        template += f'addi x26, x26, 1\n' # j++
        template += self.store_registers(free_regs=[18, 19], scalar_regs=[26])
        ...
        template += self.load_registers(free_regs=[18, 19], scalar_regs=[29])
        template += f'lw x25, (x29)\n' # idx
        template += self.store_registers(free_regs=[18, 19], scalar_regs=[25])

        template += f'j .LOOP8\n'

        template += f'.SKIP18\n'

        # Store visited_cnt to spad
        template += f'li x1, {configs.spad_addr}\n'
        template += f'ld x17, {self.get_arg_offset(self.visited_cnt_addr)}(x1)\n'
        template += f'bgt UTHREADID, x0, .SKIP29\n'
        template += self.load_registers(free_regs=[18, 19], scalar_regs=[22])
        template += f'sw x22, (x17)\n'
        template += f'.SKIP29\n'

        ################ SYNC12 #################
        template += f'KERNELBODY:\n'  # KERNELBODY12
        template += self.load_registers(free_regs=[18, 21], scalar_regs=[19, 20])
        template += f'li x1, {configs.spad_addr}\n'
        template += f'ld x17, {self.get_arg_offset(self.visited_cnt_addr)}(x1)\n'
        # Set visited_table (x19) with visited_list (x20), visited_cnt (x22)
        template += f'lw x22, (x17)\n'  # visited_cnt
        template += f'addi x26, UTHREADID, 0\n' # idx
        template += f'li x23, -1\n'
        template += f'.LOOP26\n'
        ...
        # Add acc_visited_cnt & get sorted neighbors
        template += f'bgt UTHREADID, x0, .SKIP10\n'
        template += self.load_registers(free_regs=[11, 12], scalar_regs=[7, 15, 16, 22])
        ...
        template += f'beqz x21, .SKIP10\n'

        template += f'.LOOP23\n'
        # PqPop()
        # if (*size == 0) return;
        template += f'blez x21, .SKIP23\n'

        template += f'lw x26, (x16)\n'  # nodeid
        template += f'lw x27, 4(x16)\n' # distance
        ...
        template += f'sw x27, (x20)\n'

        # if (*size == 0) return;
        template += f'beqz x21, .SKIP23\n'
        # cuda_scalar tail_dist = pq[*size].distance;
        template += f'muli x3, x21, {3 * data_size}\n'
        template += f'add x28, x16, x3\n'
        template += f'lw x3, (x28)\n'  # pq[*size].distance
        ...
        # while (r < *size) {
        template += f'.LOOP24\n'
        template += f'bge x13, x21, .SKIP24\n'
        # if (r < (*size) - 1 and gt(pq[r + 1].distance, pq[r].distance))
        template += f'addi x28, x21, -1\n'  # (*size) - 1
        template += f'bge x13, x28, .SKIP25\n'
        template += f'muli x28, x13, {3 * data_size}\n'
        template += f'add x28, x16, x28\n'
        template += f'lw x29, (x28)\n'  # pq[r].distance
        template += f'lw x30, 12(x28)\n' # pq[r+1].distance
        template += f'bge x29, x30, .SKIP25\n'
        # r++;
        template += f'addi x13, x13, 1\n'
        template += f'.SKIP25\n'
        # if (ge(tail_dist, pq[r].distance)) break;
        ...
        template += f'add x28, x16, x28\n'
        template += f'lw x29, (x28)\n'  # pq[r].distance
        template += f'bge x3, x29, .SKIP24\n'

        # pq[p] = pq[r];
        template += f'muli x30, x10, {3 * data_size}\n'
        template += f'add x30, x16, x30\n'
        template += f'sw x29, (x30)\n'  # pq[p].distance = pq[r].distance
        template += f'lw x29, 4(x28)\n' # pq[r].nodeid
        template += f'lw x18, 8(x28)\n' # pq[r].check
        template += f'addi x10, x13, 0\n' # p = r
        template += f'muli x13, x10, 2\n' # r = p * 2
        template += f'addi x13, x13, 1\n' # r = p * 2 + 1
        template += f'sw x29, 4(x30)\n' # pq[p].nodeid = pq[r].nodeid
        template += f'sw x18, 8(x30)\n' # pq[p].check = pq[r].check
        template += f'j .LOOP24\n'
        template += f'.SKIP24\n'
        # pq[p] = pq[*size];
        template += f'muli x3, x21, {3 * data_size}\n'
        ...
        template += f'ble x21, x22, .SKIP27\n'
        template += f'addi x21, x22, 0\n'
        template += f'.SKIP27\n'
        ...
        template += f'lw x19, (x25)\n'  # cand_distances

        template += f'add x18, x26, x30\n'
        template += f'sw x24, (x18)\n'
        template += f'add x13, x27, x30\n'
        template += f'sw x19, (x13)\n'

        template += f'addi x29, x29, 1\n'
        template += f'j .LOOP25\n'
        template += f'.SKIP28\n'
        ...
        template += f'KERNELBODY:\n'  # KERNELBODY14
        template += f'.SKIP10\n'
        template += self.load_registers(free_regs=[11, 12], scalar_regs=[14])
        template += f'addi x14, x14, 1\n'
        template += self.store_registers(free_regs=[11, 12], scalar_regs=[14])
        template += f'j .LOOP0\n'
